...
class PolishNeuron(Neuron):
    ...
    #                     logger.warning(f"Received 429 error. Retrying {retry_count}/{max_retries}...")
    #                     # 这里可以添加重置环境变量的逻辑
    #                     try:
    ...
    #                         logger.error(f"Failed to reset environment variables: {reset_error}")
                
    #             logger.warning(f"Polish Node: {content.survey_title} failed, {e}, retrying...")
    #             content.failure_count += 1
    #             raise
        
    #     raise Exception(f"Max retries ({max_retries}) exceeded for content: {content.survey_title}")