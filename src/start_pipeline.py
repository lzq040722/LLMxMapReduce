...
def main():
    ...
    if args.topic:
        ...
        retriever = LLM_search(model='gemini-2.0-flash-thinking-exp-01-21', infer_type="Google", engine='google')
        queries = retriever.get_queries(topic=args.topic, description=args.description)
        logger.info("---------Start to search pages.-------------")
        url_list = retriever.batch_web_search(queries=queries, topic=args.topic)
        url_list_output  = f'{args.topic}_url_list.txt'
        with open(url_list_output,'w') as f:
        ...
        crawler = AsyncCrawler(model="gemini-2.0-flash-thinking-exp-01-21", infer_type="Google")
        asyncio.run(
            crawler.run(
                topic=args.topic,
                url_list=url_list,
