...
class GroupModule(Module):
    ...
    def forward(self, survey: Survey):
        """
        主流程：根据分组模式对 survey.papers 进行分组，并生成 Digest。
        """
    ...
    def _llm_group_papers(self, papers, step, survey_title):
        ...
        基于大模型的智能分组。
        1. 先调用神经元（大模型）获得分组建议
        2. 解析分组结果，处理异常 bibkey
        3. 按 batch size 整理最终分组
        """
    ...
    def _parse_group_result(self, raw_result, bibkeys):
        """
        解析大模型返回的分组字符串，提取每组的 bibkey 列表。
        并检查是否有 hallucinate（虚构）bibkey，过滤掉。
        返回：分组结果和剩余未分组的 bibkey。
        """
        result = []
