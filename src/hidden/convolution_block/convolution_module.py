...
class ConvolutionLayerModule(Module):
    def __init__(
        ...
        self.eval_neuron = EvalOutlineNeuron(config["eval"]) #对outline进行评估
    ...
    def forward(
        ...
        #对初始建议和原始大纲进行第一次卷积操作，得到一组新的结果，包括修改后的建议、大纲和得分。
        utilise_results = [result[0] for result in conv_results_old] #前一层得到的feedback列表，作为修改大纲的初始建议
        ...
        #Survey Bias and Fairness in LLMs, block cycle count 0, Convolution Layer: 0 Convolution Finished
        #Current Layer Outline Scores: [6.7, 7.7, 8.25, 7.6, 8.8, 7.2, 8.0, 7.2, 9.57, 7.4], Avg Score: 7.842000000000001, Max Score: 9.57
...
class ConvolutionModule(Module):
    ...
    def forward(
        ...
        new_suggestion = self.conv_neuron(title, origin_outline, suggestions, bibkeys) #将片面的的suggestions整合
